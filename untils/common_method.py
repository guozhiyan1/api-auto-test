...
def delta_datetime(days=None, months=None, years=None, hours=None, minutes=None, seconds=None):
    """
    :param days:
    :param months:
    :param years:
    ...
    if days:
        now_date = now_date + relativedelta(days=days)

    if months:
        now_date = now_date + relativedelta(months=months)

    if years:
        now_date = now_date + relativedelta(years=years)

    if hours:
        now_date = now_date + relativedelta(hours=hours)

    if minutes:
        now_date = now_date + relativedelta(minutes=minutes)

    if seconds:
        now_date = now_date + relativedelta(seconds=seconds)
...
def random_name():
    ...
    # 男孩名字
    boy = '伟刚勇毅俊宝华锐瑞睿蕊芮峰强军平保东文辉力明永健世广志义兴良海山仁波宁贵福生龙元全国胜学祥才发武新利清飞彬富顺信子杰涛昌成康星光天达安岩中茂进林有坚和彪博诚先敬震振壮会思群豪心邦承乐绍功松善厚庆磊民友裕河哲江超浩亮政谦亨奇固之轮翰朗伯宏言若鸣朋斌梁栋维启克伦翔旭鹏泽晨辰士以建家致树炎德行时泰盛雄琛钧冠策腾楠榕风航弘'
    # 名
